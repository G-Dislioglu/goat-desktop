...
stage2_controlled_text_input_smoke_2026_05_27:
"Stage-2 Live-Smoke in kontrolliertem Tk-Testfenster abgeschlossen. /healthz vorab ok. Testfeld-BBox [274,398,794,439]. /action/preview type -> `Freigabe fuer Eingabe`, mayExecute=false, mouse=false, keyboard=false. /action/stage2/text ohne Freigabe -> `preview_required`, executed=false. Mit safe_text_context=false trotz user_approved -> `preview`, executed=false. Mit user_approved=true und safe_text_context=true -> `executed=true`, stage=2, target x=534/y=418, mouse=true, keyboard=true. Feldinhalt danach exakt `GOAT safe input`. Keine echten Nutzerfelder, kein Klick ausser Fokus im kontrollierten Testfeld, keine Screenshot-/Audio-Artefakte."

stage2_popup_e2e_builder_cue_2026_05_27:
"Lokaler `/builder-cue`-Pfad fuer Builder-/Test-Cues ergaenzt. Der Endpoint prueft lokale Geometrie gegen das aktive Fenster, emittiert nur einen Vorschlag ins Popup und fuehrt keine Desktop-Aktion aus (`desktop=false`, `mouse=false`, `keyboard=false`). Popup-E2E-Smoke in kontrolliertem Tk-Testfenster: `/builder-cue` -> ok true, scope `local_builder_cue_proposal`, safety_state accept; GOAT-Popup per UIA `Pruefen` -> Stage-2 Preview; UIA `Ausfuehren` -> Stage-2 Eingabe ins Testfeld. Feldinhalt danach exakt `GOAT popup E2E`, BBox [254,378,794,419]. Keine echten Nutzerfelder, keine Screenshots, keine Audioartefakte. Tests gruen: 220 passed; compileall gruen."
